...
static void init() {
	float pos[] = { -0.25, 0.25,    -0.25, -0.25,    0.25, 0.25,    0.25, -0.25};
...
static void display() {
	...
	rotateZ(&csMatrix, 45);
	translate(&csMatrix, 0.5, 0.0, 0.0);
	glUniformMatrix4fv(csMatrixLoc, 1, GL_TRUE, csMatrix.values);
	glUniform3f(squareColorLoc, 0.5, 0.1, 0.6);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);

	translate(&csMatrix, 0.0, 0.25, 0.0);
	...
	mIdentity(&csMatrix);
	rotateZ(&csMatrix, 45);
	scale(&csMatrix, 0.5, 0.5, 0.4);
	glUniformMatrix4fv(csMatrixLoc, 1, GL_TRUE, csMatrix.values);
	glUniform3f(squareColorLoc, 0.2, 0.4, 0.8);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
