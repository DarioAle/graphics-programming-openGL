...
//                          Color    cutOffInterno   Position   Exponent   direction cos(cutoff)
static float lights[]   = { 1, 0, 0,  0.8660,        -2, 0, 0,     128,      -1, 0, 0,    0.5000,
		                    0, 1, 0,  0.9659,         0, 0, 0,     128,       0, 0,-1,    0.8660,
...
static void initShaders() {
	GLuint vShader = compileShader("shaders/phong.vsh", GL_VERTEX_SHADER, GL_TRUE);
	if(!shaderCompiled(vShader)) return;
	GLuint fShader = compileShader("shaders/phong.fsh", GL_FRAGMENT_SHADER,GL_TRUE);
	...
	vShader = compileShader("shaders/position_mvp.vsh", GL_VERTEX_SHADER, GL_TRUE);
	if(!shaderCompiled(vShader)) return;
	fShader = compileShader("shaders/modelColor.fsh", GL_FRAGMENT_SHADER, GL_TRUE);
	if(!shaderCompiled(fShader)) return;
